...
Base URL (Gateway direct): `http://localhost:8000`
...
Health checks (`GET /healthz` on each agent's `base_url`) are fired concurrently with a 3-second timeout. In the learning branch, the agent `base_url` values point at standalone service containers on the Docker network.
